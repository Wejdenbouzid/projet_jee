    ...
    <form class="form-inline my-2 my-lg-0" action="login.html">
	  <button class="btn btn-outline-success my-2 my-sm-0">Logout</button>
            ...
            <tr>
                ...
                <th scope="col">Annuler Emprunt</th>
            </tr>
            </thead>
        <%
            try {
...
//connexion avec la base de données
                ...
                String sql = "SELECT * FROM `livre`,`emprunt`,`reservation` WHERE `reservation`.`IDRES`=`emprunt`.`IDRES` AND `livre`.`ISBN`=`reservation`.`ISBN`;";
                Statement stm = cx.createStatement();
                ResultSet res = stm.executeQuery(sql);
                while(res.next()){
                int id = res.getInt("IDEMP");
          %>
        <tr>
            <td><%=id%></td>
            <td><%= res.getString("DATEEMP")%></td>
            <td><%= res.getString("DATERETOUR")%></td>
            <td><%= res.getString("IDRES")%></td>
            <td><%= res.getString("DATERES")%></td>
            <td><%= res.getString("ISBN")%></td>
            <td><%= res.getString("TITRE")%></td>
            <td><%= res.getString("AUTEURS")%> </td>
            <td><%= res.getString("IDPERS")%> </td>
           <%String lien ="deleteloan.jsp?id=" + id;%>
             <td><a href ="<%=lien%>">supprimer emprunt</a></td>  
